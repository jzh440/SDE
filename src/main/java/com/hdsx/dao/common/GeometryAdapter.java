...
public class GeometryAdapter extends SeShape {
	...
	private SeCoordinateReference DEFAULT_COORD_REF = DefaultCoordRef();

	/**
	 * Adapts a JTS Geometry to a SDE Shape.
	 * <p>
	 * 
	 * <b>Note:</b> The coordinate reference defaults to <i>BCAlbers</i> !
	 * 
	 * @param geometry
	 *            A JTS Geometry to adapt
	 */
	 ...
	 *            A JTS Geometry to adapt
	 * @param coordRef
	 *            Adaptee's <code>CoordinateReference</code>
	 */
	public GeometryAdapter(Geometry geometry, SeCoordinateReference coordRef)
			throws SeException {
	 ...
	 * @throws SeException
	 */
	public static SeCoordinateReference DefaultCoordRef() throws SeException {
		SeCoordinateReference coordRef82 = new SeCoordinateReference();
		coordRef82.setCoordSysByDescription("PROJCS[\"PCS_Albers\","
	...
	/**
	 * Adapt a JTS Geometry to a SDE Shape.
	 * <p>
	 * 
	...
	protected void adapt(Geometry geometry) throws SeException {
		...
		// Use #getClass rather than instanceof, becuase LinearRing is a
		// subclass
	...
	/**
	 * Adapts a JTS <code>Polygon</code> to a SDE POLYGON. This method generate
	 * a POLYGON based on the Polygon's shell. It then loops through its holes
	 * (islands) and they are not empty, then a new island is added to the
	 * POLYGON shape.
	 * 
	 * @param polygon
	 *            A JTS <code>Polygon</code> to adapt
	 */
	protected void adapt(Polygon polygon) throws SeException {
		...
		// With the 8.2 API, must use #addPart, not #generatePolygon, before
		// adding
		...
		for (int i = 0; i < polygon.getNumInteriorRing(); i++) {
			...
			if (holeCoordinates.length > 0) {
				...
				// toString(adapt(hole.getCoordinates())));
	 ...
	 * 
	 * @param multiPolygon
	 *            A JTS <code>MultiPolygon</code> to adapt
	 */
	 ...
	 * 
	 * @param point
	 *            A JTS <code>Point</code> to adapt
	 */
	protected void adapt(com.vividsolutions.jts.geom.Point point)
			throws SeException {
		generatePoint(1,
				new SDEPoint[] { new CoordinateAdapter(point.getCoordinate()) });
	}
	 ...
	 * 
	 * @param multiPoint
	 *            A JTS <code>MultiPoint</code> to adapt
	 */
	 ...
	 * 
	 * @param lineString
	 *            A JTS <code>LineString</code> to adapt
	 */
	 ...
	 *            A JTS <code>MultiLineString</code> to adapt
	 */
	...
	/**
	 * Adapts a <code>MultiLineString</code>, <code>MultiPoint</code>, or
	 * <code>MultiPolygon</code> respectively to a SDE MULTI_LINE, MULTI_POINT,
	 * or MULTI_POLYGON.
	 * 
	 * @param geometryCollection
	 *            A JTS <code>GeometryCollection</code> to adapt
	 */
	private void adapt(GeometryCollection geometryCollection)
			throws SeException {

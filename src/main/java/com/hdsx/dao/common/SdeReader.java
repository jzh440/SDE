 ...
 * <li>Inverted Polygons and Exverted Holes are read as is. These create invalid
 * JTS polygons, and must be rectified if further operations are to be performed
 * on them.
 * </ul>
 * <p>
 * This class allows specifying the maximum number of coordinate dimensions to
 * read. If dimensions higher than 2 are not required, it may be more efficient
 * to not read them.
 * <p>
...
public class SdeReader {
	...
	/**
	 * Creates a reader that creates geometries using the default
	...
	/**
	 * Creates a reader that creates geometries using the given
	 * {@link GeometryFactory}.
	 * 
	...
	/**
	 * Sets the maximum number of coordinate dimensions to read. If this is
	 * larger than the number of dimensions actually present in the input
	 * geometry, the higher ordinates will not be read, and NaN will be returned
	 * as their value.
	 * <p>
	 ...
	 * @throws SeException
	 */
	public Geometry read(SeShape shape) throws SeException {
		...
		throw new IllegalArgumentException("Shapes of type " + shape.getType()
				+ " are not supported");
	}

	private Point readPoint(SeShape shape) throws SeException {
		java.util.List list = shape.getAllPoints(SeShape.TURN_RIGHT, false); // get
																				// points
																				// and
																				// offsets
	...
	private MultiPoint readMultiPoint(SeShape shape) throws SeException {
		java.util.List list = shape.getAllPoints(SeShape.TURN_RIGHT, false); // get
																				// points
																				// and
																				// offsets
		SDEPoint[] sePts = (SDEPoint[]) list.get(0);
		return geometryFactory.createMultiPoint(toCoordinates(sePts, 0,
				sePts.length));
	}

	private LineString readLine(SeShape shape) throws SeException {
		java.util.List list = shape.getAllPoints(SeShape.TURN_RIGHT, false); // get
																				// points
																				// and
																				// offsets
		SDEPoint[] sePts = (SDEPoint[]) list.get(0);
		return geometryFactory.createLineString(toCoordinates(sePts, 0,
				sePts.length));
	}

	private MultiLineString readMultiLine(SeShape shape) throws SeException {
		java.util.List list = shape.getAllPoints(SeShape.TURN_RIGHT, false); // get
																				// points
																				// and
																				// offsets
		...
		for (int i = 0; i < partOffset.length; i++) {
			...
			lines[i] = geometryFactory.createLineString(toCoordinates(sePts,
					partOffset[i], end));
		}
		return geometryFactory.createMultiLineString(lines);
	}

	private Polygon readPolygon(SeShape shape) throws SeException {
		java.util.List list = shape.getAllPoints(SeShape.TURN_RIGHT, true); // get
																			// points
																			// and
																			// offsets
		SDEPoint[] sePts = (SDEPoint[]) list.get(0);
	...
	private Polygon readPolygon(SDEPoint[] sePts, int[] subPartOffset,
			int subPartStart, int subPartEnd) throws SeException {
		...
		for (int i = subPartStart; i < subPartEnd; i++) {
			...
			LinearRing ring = geometryFactory.createLinearRing(toCoordinates(
					sePts, subPartOffset[i], end));
	...
	private MultiPolygon readMultiPolygon(SeShape shape) throws SeException {
		java.util.List list = shape.getAllPoints(SeShape.TURN_RIGHT, true); // get
																			// points
																			// and
																			// offsets
		...
		for (int i = 0; i < partOffset.length; i++) {
			...
			polys[i] = readPolygon(sePts, subPartOffset, partOffset[i],
					subPartEnd);
		}
	...
	private CoordinateSequence toCoordinates(SDEPoint[] sePts, int start,
			int end) throws SeException {
		int size = end - start;

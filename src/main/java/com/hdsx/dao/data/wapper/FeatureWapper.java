...
import com.hdsx.dao.common.SdeReader;
...
public class FeatureWapper implements IFeatureWapper {
	...
	public Feature SeRow2Feature(SeRow row,boolean returnGeometry) throws Exception {
		...
		for(int i=0,num=row.getNumColumns();i<num;i++){
			...
			if(columnType==SeColumnDefinition.TYPE_SHAPE&&returnGeometry){
				if(row.getShape(i)!=null){
					sdeReader=new SdeReader();
					feature.setShape(sdeReader.read(row.getShape(i)));
				}
			}else{
				JavaType jt=JavaType.sde2JavaType(columnType);
